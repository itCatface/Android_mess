package cc.catface.clibrary.util.view.viewpager.banner;
...
import cc.catface.clibrary.util.view.viewpager.banner.adapter.BaseBannerAdapter;
...
public abstract class RecyclerViewBannerBase<L extends RecyclerView.LayoutManager, A extends BaseBannerAdapter> extends FrameLayout {
    ...
    protected boolean showTitleBar;// 是否显示标题背景条
    ...
    protected OnBannerScrolledListener onBannerScrolledListener;
    ...
    protected void initView(Context context, AttributeSet attrs) {
        ...
        showTitleBar = a.getBoolean(R.styleable.RecyclerViewBannerBase_showTitleBar, true);
        titleBarHeight = a.getInt(R.styleable.RecyclerViewBannerBase_titleBarHeight, 30);
        ...
        int marginRight = a.getDimensionPixelSize(R.styleable.RecyclerViewBannerBase_indicatorMarginRight, dp2px(16));
        int marginBottom = a.getDimensionPixelSize(R.styleable.RecyclerViewBannerBase_indicatorMarginBottom, dp2px(9));
        int g = a.getInt(R.styleable.RecyclerViewBannerBase_indicatorGravity, 2);
        int gravity;
        if (g == 0) {
    ...
    private void addTitleBar(Context context) {
        RelativeLayout titleBar = new RelativeLayout(context);
        titleBar.setBackgroundColor(getColor(R.color.banner_title_bg));
        LayoutParams paramsTitleBar = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp2px(titleBarHeight));
        paramsTitleBar.gravity = Gravity.BOTTOM;
        addView(titleBar, paramsTitleBar);
        if (!showTitleBar) {
     ...
     * 设置轮播数据集
     */
    public void initBannerImageView(@NonNull List<String> newList, OnBannerItemClickListener onBannerItemClickListener, OnBannerScrolledListener onBannerScrolledListener) {
        this.onBannerScrolledListener = onBannerScrolledListener;
        ...
        if (compareListDifferent(newList, tempUrlList)) {
            ...
            if (bannerSize > 1) {
                ...
                // 首次展示第一个标题
                if (null != onBannerScrolledListener) onBannerScrolledListener.onBannerScrolled(0);
    ...
    protected synchronized void refreshIndicator() {
        if (showIndicator && bannerSize > 1) {
            ...
            if (null != onBannerScrolledListener) onBannerScrolledListener.onBannerScrolled(currentIndex % bannerSize);
            indicatorAdapter.notifyDataSetChanged();
        }
    }
    ...
    protected boolean compareListDifferent(List<String> newTabList, List<String> oldTabList) {
        ...
        for (int i = 0; i < newTabList.size(); i++) {
            if (TextUtils.isEmpty(newTabList.get(i))) return true;
