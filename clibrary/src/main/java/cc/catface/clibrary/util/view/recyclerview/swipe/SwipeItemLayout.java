package cc.catface.clibrary.util.view.recyclerview.swipe;
...
public class SwipeItemLayout extends FrameLayout {
    ...
    @Override public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            if (isCloseAnimating()) // 关闭菜单过程中禁止接收down事件
                return false;

            if (mIsOpen && isTouchContent(((int) ev.getX()), ((int) ev.getY()))) {  // 菜单开启时，点击条目关闭菜单
                close();
                return false;
            }
        }
    ...
    @Override public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!beforeCheckDrag && mIsDragged) {    // 开始拖动后，发送一个cancel事件用来取消点击效果
            ...
            case MotionEvent.ACTION_CANCEL:
            case MotionEvent.ACTION_UP:
                if (mIsDragged || mIsOpen) {    // 拖拽后手指抬起，或者已经开启菜单，不应该响应到点击事件
                    mDragHelper.processTouchEvent(ev);
     ...
     * 判断是否可以拖拽View
     */
    @SuppressLint("RtlHardcoded") private void checkCanDragged(MotionEvent ev) {
        if (mIsDragged)
            return;
    ...
    @SuppressLint("RtlHardcoded") @Override public void addView(View child, int index, ViewGroup.LayoutParams params) {
    ...
    @SuppressLint("RtlHardcoded") private boolean isLeftMenu() {
        return mCurrentMenu != null && mCurrentMenu == mMenus.get(Gravity.LEFT);
    }

    @SuppressLint("RtlHardcoded") private boolean isRightMenu() {
